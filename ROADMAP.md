...
- 2026-05-30 — **Source-hash-keyed incremental retranslation.** `scripts/auto-translate.mjs` now writes a `{target}.hashes.json` map (16-hex SHA-256 per source leaf) beside each draft and reuses a translation only when the English source hash still matches; an edited `en.json` string re-translates exactly that key instead of silently shipping the pre-edit es/ja/zh value, so `--fresh` is no longer needed for routine edits. A run with no hash map yet reuses every present key and bootstraps the map (no mass re-translate on upgrade). Pure helpers (`sha` / `hashTree` / `shouldReuse`) are exported and `main()` is guarded for importability. Verified end-to-end: one edited key re-translated only itself (262 others reused untouched); a hashless run translated nothing and wrote the map. research(2026-05): content-addressed caches invalidate on input change, not key presence — keying reuse on a source hash is the standard incremental-rebuild pattern.
