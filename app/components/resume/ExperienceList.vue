...
function bulletsFor(id: string): string[] {
  const raw = tm(`timeline.${id}.bullets`)
  ...
  <section class="space-y-6">
    ...
    <div v-if="viewMode === 'industry' && professionalExperience?.length">
      <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
        ...
        {{ $t('resume.sections.professional') }}
      </h2>
      <div class="space-y-4">
        <article
        ...
        >
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
            <div>
              ...
              >
                {{ $t('timeline.' + exp.id + '.title') }}
                <span
                  v-if="exp.isCurrent"
                  class="px-2 py-0.5 text-xs font-semibold bg-green-700 text-white rounded-full shadow-sm"
                >
          ...
          <ul class="list-disc list-inside space-y-1 text-gray-600 dark:text-gray-300 text-sm">
            <li v-for="(bullet, idx) in bulletsFor(exp.id)" :key="idx">{{ bullet }}</li>
          </ul>
        </article>
      </div>
    </div>
    ...
    <div v-if="viewMode === 'research' && researchExperience?.length">
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
            <div>
              ...
              <p v-if="exp.project" class="text-sm text-gray-500 dark:text-gray-400">
                {{ $t('resume.experience.projectPrefix') }} {{ exp.project }}
              </p>
            </div>
            <div class="text-sm text-gray-500 dark:text-gray-400 sm:text-right">
              <p>{{ exp.startDate }} – {{ exp.endDate }}</p>
          ...
          <ul
            v-if="bulletsFor(exp.id).length"
            class="list-disc list-inside space-y-1 text-gray-600 dark:text-gray-300 text-sm"
          >
            <li v-for="(bullet, idx) in bulletsFor(exp.id)" :key="idx">{{ bullet }}</li>
          </ul>
        </article>
